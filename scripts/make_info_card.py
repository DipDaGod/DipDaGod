    ...
    "sections": [
        {
            "title": "Languages",
            "kv": {
                "<LANGUAGE_1>": "<SKILL_LEVEL>",
                "<LANGUAGE_2>": "<SKILL_LEVEL>",
                "<LANGUAGE_3>": "<SKILL_LEVEL>",
                "<LANGUAGE_4>": "<SKILL_LEVEL>",
            },
        },
        {
            "title": "Now",
...
def compute_card_width(config: dict, value_x: float) -> float:
    ...
    for value in values:
        candidate_widths.append(value_x + len(value) * VALUE_CHAR_WIDTH_ESTIMATE + PADDING)

    for section in config.get("sections", []):
        for bullet_text in section.get("bullets", []):
            candidate_widths.append(
                KEY_X + 14 + len(bullet_text) * VALUE_CHAR_WIDTH_ESTIMATE + PADDING
            )

    title_line = f'{config["username"]}@{config["host"]}: ~$ {config.get("command", "neofetch")}'
    candidate_widths.append(len(title_line) * 7.0 + PADDING * 4)  # centered, needs margin both sides
